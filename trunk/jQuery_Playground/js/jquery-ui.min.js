...
$.fn.dialogInit=function(o){return this.each(function(){if(!$(this).is(".ui-dialog")){$.ui.dialogInit(this,o);}});}
...
return selected>=0?++selected:-1;};$.ui.tabs=function(el,options){this.source=el;this.options=$.extend({initial:0,event:'click',disabled:[],unselected:false,unselect:options.unselected?true:false,spinner:'Loading&#8230;',cache:false,idPrefix:'tab-',fxSpeed:'normal',add:function(){},remove:function(){},enable:function(){},disable:function(){},click:function(){},hide:function(){},show:function(){},load:function(){},tabTemplate:'<li><a href="#{href}"><span>#{text}</span></a></li>',panelTemplate:'<div></div>',navClass:'ui-tabs-nav',selectedClass:'ui-tabs-selected',disabledClass:'ui-tabs-disabled',panelClass:'ui-tabs-panel',hideClass:'ui-tabs-hide',loadingClass:'ui-tabs-loading'},options);this.options.event+='.ui-tabs';var uuid='tabs'+$.ui.tabs.prototype.count++;$.ui.tabs.instances[uuid]=this;$.data(el,'tabsUUID',uuid);this.tabify(true);};$.ui.tabs.instances={};$.ui.tabs.getInstance=function(el){return $.ui.tabs.instances[$.data(el,'tabsUUID')];};$.extend($.ui.tabs.prototype,{count:0,tabId:function(a,i){return a.title?a.title.replace(/\s/g,'_'):this.options.idPrefix+this.count+'-'+(i+1);},tabify:function(init){this.$tabs=$('a:first-child',this.source);this.$panels=$([]);var self=this,o=this.options;this.$tabs.each(function(i,a){if(a.hash&&a.hash.replace('#','')){self.$panels=self.$panels.add(a.hash);}
else{$.data(a,'href',a.href);var id=self.tabId(a,i);a.href='#'+id;self.$panels=self.$panels.add($('#'+id)[0]||$(o.panelTemplate).attr('id',id).addClass(o.panelClass).insertAfter(self.$panels[i-1]||self.source));}});if(init){this.$tabs.each(function(i,a){if(location.hash){if(a.hash==location.hash){o.initial=i;if($.browser.msie||$.browser.opera){var $toShow=$(location.hash),toShowId=$toShow.attr('id');$toShow.attr('id','');setTimeout(function(){$toShow.attr('id',toShowId);},500);}
scrollTo(0,0);return false;}}else if($(a).parents('li:eq(0)').is('li.'+o.selectedClass)){o.initial=i;return false;}});$(this.source).is('.'+o.navClass)||$(this.source).addClass(o.navClass);this.$panels.each(function(){var $this=$(this);$this.is('.'+o.panelClass)||$this.addClass(o.panelClass);});var $lis=$('li',this.source);this.$panels.addClass(o.hideClass);$lis.removeClass(o.selectedClass);if(!o.unselected){this.$panels.slice(o.initial,o.initial+1).show();$lis.slice(o.initial,o.initial+1).addClass(o.selectedClass);}
var href=this.$tabs[o.initial]&&$.data(this.$tabs[o.initial],'href');if(href){this.load(o.initial+1,href);}
...
this.$tabs.unbind(o.event).bind(o.event,function(){var $li=$(this).parent('li'),$hide=self.$panels.filter(':visible'),$show=$(this.hash);if(($li.is('.'+o.selectedClass)&&!o.unselect)||$li.is('.'+o.disabledClass)||o.click(this,$show[0],$hide[0])===false){this.blur();return false;}
if(o.unselect){if($li.is('.'+o.selectedClass)){$li.removeClass(o.selectedClass);self.$panels.stop();hideTab(this,$hide);this.blur();return false;}else if(!$hide.length){$li.addClass(o.selectedClass);self.$panels.stop();showTab(this,$show);this.blur();return false;}}
...
this.blur();return false;});},add:function(url,text,position){if(url&&text){var o=this.options;position=position||this.$tabs.length;var $li=$(o.tabTemplate.replace(/#\{href\}/,url).replace(/#\{text\}/,text));if(url.indexOf('#')==0){var id=url.replace('#','')}else{var id=this.tabId($('a:first-child',$li)[0],position);}
var $panel=$('#'+id);$panel=$panel.length&&$panel||$(o.panelTemplate).attr('id',id).addClass(o.panelClass).addClass(o.hideClass);if(position>=this.$tabs.length){$li.appendTo(this.source);$panel.appendTo(this.source.parentNode);}else{$li.insertBefore(this.$tabs.slice(position-1,position).parent('li'));$panel.insertBefore(this.$panels[position-1]);}
